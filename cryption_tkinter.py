...
class __tk__:
    ...
    def func(selftk, event):
        ...
        selftk.wb.save("cryption.xlsx")  # saves the file everytime it writes on it
        selftk.n = selftk.n + 1
    ...
    def create(selftk):
        selftk.root.geometry('700x700+100+2')
        selftk.root.title('CRYPTION')
        selftk.send()
        ...
        l1 = Label(f, text='Please enter your text here: ', padx=5, pady=5, font=('Calibri', 10, 'bold'), relief=GROOVE,
                   borderwidth=3, bg='#F8A466')
        l1.pack(anchor=NW, padx=15, pady=5)
        l2 = Label(f1, text='Your crypted text is: ', padx=5, pady=5, font=('Calibri', 10, 'bold'), relief=GROOVE,
                   borderwidth=3, bg='#F8A466')
        ...
        b = Button(f2, text='CRYPT', padx=10, pady=10, bg='sky blue', font=('Calibri', 11, 'bold'))
        b.pack()
...
wb = openpyxl.load_workbook("E:\\PYTHON\\cryption codes\\tkinter\\cryption.xlsx")
sheet = wb.active
if sheet.max_row == 1:
    n = sheet.max_row
else:
